...
.PARAMETER UrlAllowlist
    Array of URLs to allowlist. Default: @("https://*.kromannreumert.com/*")
...
.PARAMETER CompanyName
    Company name for branding. Default: "Kromann Reumert"

.PARAMETER CompanyURL
    Company URL for branding. Default: "https://kromannreumert.com"

.PARAMETER ProductName
    Product name for branding. Default: "Kromann Reumert IT"

.PARAMETER SupportEmail
    Support email for branding. Default: "hlp@kromannreumert.com"

.PARAMETER PrimaryColor
    Primary color for branding (hex format). Default: "#0078D4"

.PARAMETER LogoUrl
    Logo URL for branding. Default: "https://krpublicfiles.blob.core.windows.net/toastnotification/logo.jpg"

.NOTES
    Author: Martin Bengtsson
    Blog: https://www.imab.dk
    Organization: Kromann Reumert
...
Param(
    ...
    [string[]]$UrlAllowlist = @("https://*.kromannreumert.com/*", "https://*.kromannreumert.sharepoint.com/*", "https://*.securembly.com"), 
    ...
    # Default is "Kromann Reumert"
    [string]$CompanyName = "Kromann Reumert",
    
    # This will set the Company URL option in the Custom Branding settings
    # Default is "https://kromannreumert.com"; Must include the protocol (e.g., https://)
    [string]$CompanyURL = "https://kromannreumert.com",
    
    # This will set the "Product Name" option in the Custom Branding settings
    # Default is "Kromann Reumert IT"
    [string]$ProductName = "Kromann Reumert IT",
    
    # This will set the "Support Email" option in the Custom Branding settings
    # Default is "hlp@kromannreumert.com"
    [string]$SupportEmail = "hlp@kromannreumert.com",
    ...
    # Default is "https://krpublicfiles.blob.core.windows.net/toastnotification/logo.jpg"
    # Must be a valid URL including the protocol (e.g., https://example.com/logo.png)
    # Protocol must be https; recommended size is 48x48 pixels with a maximum of 128x128
    [string]$LogoUrl = "https://krpublicfiles.blob.core.windows.net/toastnotification/logo.jpg"
)
...
foreach ($BrowserName in $Browser) {
    ...
    # Configure URL Allowlist as JSON
    if ($UrlAllowlist.Count -gt 0) {
        $json = $UrlAllowlist | ConvertTo-Json -Compress
        Write-Output "$LogPrefix Applying URL allowlist ($($UrlAllowlist.Count) patterns)"
        Set-RegistryProperty -Path $policyPath -Name "urlAllowlist" -Value $json -Type "String" | Out-Null
    }

    # Configure Custom Branding as subkey with individual properties
    $brandingCount = 0
    if ($CompanyName) { $brandingCount++ }
    if ($CompanyURL) { $brandingCount++ }
    if ($ProductName) { $brandingCount++ }
    if ($SupportEmail) { $brandingCount++ }
    if ($PrimaryColor) { $brandingCount++ }
    if ($LogoUrl) { $brandingCount++ }
    
    if ($brandingCount -gt 0) {
        $customBrandingKey = "$policyPath\customBranding"
        
        Try {
            if (!(Test-Path $customBrandingKey)) {
                New-Item -Path $customBrandingKey -Force -ErrorAction Stop | Out-Null
            }
            
            if ($CompanyName) {
                New-ItemProperty -Path $customBrandingKey -Name "companyName" -PropertyType String -Value $CompanyName -Force -ErrorAction Stop | Out-Null
            }
            if ($CompanyURL) {
                New-ItemProperty -Path $customBrandingKey -Name "companyURL" -PropertyType String -Value $CompanyURL -Force -ErrorAction Stop | Out-Null
            }
            if ($ProductName) {
                New-ItemProperty -Path $customBrandingKey -Name "productName" -PropertyType String -Value $ProductName -Force -ErrorAction Stop | Out-Null
            }
            if ($SupportEmail) {
                New-ItemProperty -Path $customBrandingKey -Name "supportEmail" -PropertyType String -Value $SupportEmail -Force -ErrorAction Stop | Out-Null
            }
            if ($PrimaryColor) {
                New-ItemProperty -Path $customBrandingKey -Name "primaryColor" -PropertyType String -Value $PrimaryColor -Force -ErrorAction Stop | Out-Null
            }
            if ($LogoUrl) {
                New-ItemProperty -Path $customBrandingKey -Name "logoUrl" -PropertyType String -Value $LogoUrl -Force -ErrorAction Stop | Out-Null
            }
            
            Write-Output "$LogPrefix Custom branding configured successfully"
        }
        Catch {
            Write-Output "$LogPrefix ERROR: Failed to configure custom branding - $($_.Exception.Message)"
        }
    }
